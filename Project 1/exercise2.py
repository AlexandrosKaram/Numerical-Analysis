...
def calculate_results():
    ...
    Returns:
        tuple(tuple): Tuple containing the results of each method also in tuples.
    """
    bisection_2_results = (
...
def print_results_diversity():
    ...
    print("\nOut of 20 times: ")
    print(f"For Bisection: {len(results_sets[0]) - 1} different results.")
    print(f"For Newton-Raphson: {len(results_sets[1]) - 1} different results.")
    print(f"For Secant: {len(results_sets[2]) - 1} different results.")
        ...
        for j, result in enumerate(method):
            if result[1] != None and variation_results[i][j][1] != None:
                if result[1] < variation_results[i][j][1]:
                    ...
                    variation_times_better += 1

    print("\nOut of 15 results tested:")
    print(f"Variation methods appeared faster {variation_times_better} times.")
    print(f"Original methods appeared faster {original_times_better} times.")
    print(f"Both methods were equally fast {equal} times.")
    print(f"One of the two did not converge {15 - equal - variation_times_better - original_times_better} times.\n")
